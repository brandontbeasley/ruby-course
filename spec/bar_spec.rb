...
# # # # # # # # # # # # # # # # # # # # # #
  ...
  describe "During normal hours" do
    it "does not give a discount during normal hours" do
      Time.stub(:now).and_return(Time.parse("10 am"))
      @bar.add_menu_item("beer", 4.00)
      item = @bar.menu_items.first
      expect(@bar.get_price(item)).to eq(item.price)
    end
  end

  describe "During happy hours" do
    it "does give a discount during normal hours" do
      Time.stub(:now).and_return(Time.parse("3 pm"))
      discount = @bar.happy_discount = 0.5
      @bar.add_menu_item("beer", 4.00)
      item = @bar.menu_items.first
      expect(@bar.get_price(item)).to eq(2)
    end
  end
